...
Treeview]{.breadcrumb_last aria-current="page"}
:::

Tkinter Treeview {#tkinter-treeview .entry-title}
================

::: {.entry-content}
**Summary**: in this tutorial, you'll learn about the Tkinter Treeview
widget and how to use it to display both tabular and hierarchical data.
...
``` {.wp-block-code aria-describedby="shcb-language-1" data-shcb-language-name="Python" data-shcb-language-slug="python"}
     ...
     tree = ttk.Treeview(container, **options)Code language: Python (python)
```

A Treeview widget holds a list of items. Each item has one or more
columns.

The first column may contain text and an icon that indicates whether you
can expand it or not.

The remaining columns contain values that you want to display for each
row.

The first row of the Treeview consists of headings that identify each
column by its name.

Using Tkinter Treeview to display tabular data
----------------------------------------------

The following program shows how to use the Treeview widget to display
tabular data:
...
::: {.wp-block-image}
![](https://www.pythontutorial.net/wp-content/uploads/2020/12/Tkinter-Treeview-Tabular-Data.png){.wp-image-1548
width="622" height="232" sizes="(max-width: 622px) 100vw, 622px"
srcset="https://www.pythontutorial.net/wp-content/uploads/2020/12/Tkinter-Treeview-Tabular-Data.png 622w, https://www.pythontutorial.net/wp-content/uploads/2020/12/Tkinter-Treeview-Tabular-Data-300x112.png 300w"}
:::

How it works.

First, import `tkinter` module, `ttk` submodule, and the `showinfo` from
`tkinter.messagebox`:
...
``` {.wp-block-code aria-describedby="shcb-language-3" data-shcb-language-name="Python" data-shcb-language-slug="python"}
     ...
     import tkinter as tk
from tkinter import ttk
from tkinter.messagebox import showinfoCode language: Python (python)
```
...
``` {.wp-block-code aria-describedby="shcb-language-4" data-shcb-language-name="Python" data-shcb-language-slug="python"}
     ...
     root = tk.Tk()
root.title('Treeview demo')
root.geometry('620x200')Code language: Python (python)
```
...
``` {.wp-block-code aria-describedby="shcb-language-5" data-shcb-language-name="Python" data-shcb-language-slug="python"}
     ...
     columns = ('#1', '#2', '#3')Code language: Python (python)
```
...
``` {.wp-block-code aria-describedby="shcb-language-6" data-shcb-language-name="Python" data-shcb-language-slug="python"}
     ...
     tree = ttk.Treeview(root, columns=columns, show='headings')Code language: Python (python)
```

In this code, we passed the columns to the columns option. The
show='heading' hide the first column (column `#0`) of the Treeview.

The `show` option accepts one of the following values:

-   `'tree'` -- shows the column \#0
-   `'heading'` -- shows the header row
-   `'tree headings'` -- shows both column \#0 and the header row. This
...
``` {.wp-block-code aria-describedby="shcb-language-7" data-shcb-language-name="Python" data-shcb-language-slug="python"}
     ...
     tree.heading('#1', text='First Name')
tree.heading('#2', text='Last Name')
tree.heading('#3', text='Email')Code language: Python (python)
```

Sixth, generate a list of tuples for displaying on the Treeview:

``` {.wp-block-code aria-describedby="shcb-language-8" data-shcb-language-name="JavaScript" data-shcb-language-slug="javascript"}
        
            
        
     contacts = []
for n in range(1, 100):
    contacts.append((f'first {n}', f'last {n}', f'email{n}@example.com'))Code language: JavaScript (javascript)
```

Seventh, create new items, one by one, by using the `insert()` method of
the Treeview object:
...
``` {.wp-block-code aria-describedby="shcb-language-9" data-shcb-language-name="Python" data-shcb-language-slug="python"}
     ...
     for contact in contacts:
    tree.insert('', tk.END, values=contact)Code language: Python (python)
```

Eight, define a function to handle the `<>` event. When you select one
or more items, the program will show a message box:
...
tree.bind('<<TreeviewSelect>>', item_selected)Code language: Python (python)
```

Ninth, place the Treeview widget on the root window:

``` {.wp-block-code aria-describedby="shcb-language-11" data-shcb-language-name="Python" data-shcb-language-slug="python"}
        
            
        
     tree.grid(row=0, column=0, sticky='nsew')Code language: Python (python)
```

Tenth, [add a vertical
scrollbar](https://www.pythontutorial.net/tkinter/tkinter-scrollbar/) to
to the Treeview widget:

``` {.wp-block-code aria-describedby="shcb-language-12" data-shcb-language-name="PHP" data-shcb-language-slug="php"}
...
scrollbar = ttk.Scrollbar(root, orient=tk.VERTICAL, command=tree.yview)
tree.configure(yscroll=scrollbar.set)
scrollbar.grid(row=0, column=1, sticky='ns')Code language: PHP (php)
```

Finally, display the root window:

``` {.wp-block-code aria-describedby="shcb-language-13" data-shcb-language-name="Python" data-shcb-language-slug="python"}
...
The following program illustrates how to use the TreeView widget to
display hierarchical data:

``` {.wp-block-code aria-describedby="shcb-language-14" data-shcb-language-name="Python" data-shcb-language-slug="python"}
...
::: {.wp-block-image}
![](https://www.pythontutorial.net/wp-content/uploads/2020/12/Tkinter-Treeview-Hierarchical-Data.png){.wp-image-1549
width="400" height="230" sizes="(max-width: 400px) 100vw, 400px"
srcset="https://www.pythontutorial.net/wp-content/uploads/2020/12/Tkinter-Treeview-Hierarchical-Data.png 400w, https://www.pythontutorial.net/wp-content/uploads/2020/12/Tkinter-Treeview-Hierarchical-Data-300x173.png 300w"}
:::
...
``` {.wp-block-code aria-describedby="shcb-language-15" data-shcb-language-name="JavaScript" data-shcb-language-slug="javascript"}
        
            
        
     tree = ttk.Treeview(root)
tree.heading('#0', text='Departments', anchor='w')Code language: JavaScript (javascript)
```
...
``` {.wp-block-code aria-describedby="shcb-language-16" data-shcb-language-name="PHP" data-shcb-language-slug="php"}
...
tree.insert('', tk.END, text='Sales', iid=2, open=False)
tree.insert('', tk.END, text='Finance', iid=3, open=False)
tree.insert('', tk.END, text='IT', iid=4, open=False)Code language: PHP (php)
```

Each item is identified by an `iid`. If you skip the `iid`, the insert
method will generate one automatically. In this case, you need to have
explicit `iid` for adding child items.

Third, add two child items to the item with iid 0 by using the insert()
and move() methods:

``` {.wp-block-code aria-describedby="shcb-language-17" data-shcb-language-name="PHP" data-shcb-language-slug="php"}
...
tree.insert('', tk.END, text='Jane Doe', iid=6, open=False)
tree.move(5, 0, 0)
tree.move(6, 0, 1)Code language: PHP (php)
```

Finally, place the Treeview widget on the root window and display it.

``` {.wp-block-code aria-describedby="shcb-language-18" data-shcb-language-name="PHP" data-shcb-language-slug="php"}
...
-   Use a Tkinter Treeview widget to display both tabular and
    hierarchical data.
...
::: {.page-previous}
[Previous Tkinter
Notebook](https://www.pythontutorial.net/tkinter/tkinter-notebook/ "Tkinter Notebook"){.prev-page-anchor}
:::

::: {.page-next}
[Next Tkinter
Frame](https://www.pythontutorial.net/tkinter/tkinter-frame/ "Tkinter Frame"){.next-page-anchor}
:::
:::
:::
...
::: {#searchwp_live_search_results_60614bfa79d00 .searchwp-live-search-results aria-expanded="false" role="listbox" tabindex="0" style="left: 970px; top: 153px; width: 300px;"}
:::
